/**
 * datav-kit chart template — pie-doughnut (占比 / part-to-whole).
 *
 ...
 *   6. performance guard the 5-category cap + animationThreshold + the min-size guard
 *
...
function typeTheme(t) {
  return {
    pie: {
      label: { color: withAlpha(t.primary, 0.85), fontSize: t.fontXs, fontFamily: t.fontFamily },
      labelLine: { lineStyle: { color: withAlpha(t.primary, 0.45), width: t.lineWidth } },
    },
  }
}
...
export function createPieDoughnut(el, data, tokens = readDatavTokens(el)) {
  let chart = null
  let overlay = null
