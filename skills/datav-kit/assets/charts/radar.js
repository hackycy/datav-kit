/**
 * datav-kit chart template — radar (多维 / multidimensional).
 *
 ...
 *   6. performance guard the 5-indicator cap + animationThreshold + the min-size guard
 *
...
function typeTheme(t) {
  return {
    radar: {
      axisName: { color: withAlpha(t.primary, 0.72), fontSize: t.fontXs, fontFamily: t.fontFamily },
      axisLine: { lineStyle: { color: withAlpha(t.primary, 0.18), width: t.lineWidth } },
      splitLine: { lineStyle: { color: withAlpha(t.primary, 0.18), width: t.lineWidth } },
...
export function createRadar(el, data, tokens = readDatavTokens(el)) {
  let chart = null
  let overlay = null
