/**
 * datav-kit chart template — line-area (时序 / time series).
 *
 ...
 *   6. performance guard sampling: 'lttb' + animationThreshold + the 4-line cap
 *
...
function typeTheme(t) {
  return { line: { lineStyle: { width: 2, shadowBlur: t.glowBlur, shadowColor: t.glowColor } } }
}
...
function buildOption(data, t) {
  ...
  return {
    ...
    // Explicit grid: the library default (10% / 60) wastes most of a small panel.
    grid: {
      left: 8,
      right: 12,
      top: series.length > 1 ? 28 : 8,
      bottom: 8,
...
export function createLineArea(el, data, tokens = readDatavTokens(el)) {
  let chart = null
  let overlay = null
  ...
  function degrade() {
    ...
    chart = null
    overlay = null
    const value = (data.series?.[0]?.data || []).find(value => typeof value === 'number') ?? 0
