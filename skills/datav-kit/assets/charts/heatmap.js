/**
 * datav-kit chart template — heatmap (密度 / density).
 *
 ...
 *   6. performance guard progressive rendering + animationThreshold + the two-category-axis rule
 *
...
function typeTheme(t) {
  return {
    visualMap: { textStyle: { color: withAlpha(t.primary, 0.72), fontSize: t.fontXs, fontFamily: t.fontFamily } },
  }
}
...
function buildOption(data, t) {
  ...
  const numbers = cells.map(cell => cell[2]).filter(value => typeof value === 'number')
  const min = data.min ?? numbers.reduce((acc, value) => Math.min(acc, value), 0)
  const max = data.max ?? numbers.reduce((acc, value) => Math.max(acc, value), 1)

  return {
    animation: !prefersReducedMotion(),
    animationDuration: CHART_MOTION,
    animationThreshold: 2000,
    // Explicit grid: the bottom gutter holds the horizontal visualMap.
    grid: { left: 8, right: 8, top: 8, bottom: 40, outerBoundsMode: 'same', outerBoundsContain: 'axisLabel' },
    tooltip: { position: 'top' },
    // A heatmap needs two category axes and a visualMap (ECharts requirement).
    xAxis: { type: 'category', data: data.xLabels || [], splitArea: { show: false } },
    yAxis: { type: 'category', data: data.yLabels || [], splitArea: { show: false } },
    visualMap: {
      min,
      max,
      calculable: false,
    ...
    series: [{
      ...
      label: { show: false },
      itemStyle: { borderColor: withAlpha(t.primary, 0.12), borderWidth: t.lineWidth },
    }],
  }
}
...
export function createHeatmap(el, data, tokens = readDatavTokens(el)) {
  let chart = null
  let overlay = null
