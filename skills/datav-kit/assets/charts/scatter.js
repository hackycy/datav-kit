/**
 * datav-kit chart template — scatter (分布 / distribution).
 *
 ...
 *   6. performance guard large + largeThreshold(2000) + animationThreshold
 *
...
function buildOption(data, t) {
  ...
  if (points > LARGE_THRESHOLD)
    console.warn(`[datav-kit] scatter: ${points} points enter large mode; per-point symbol sizes are dropped above ${LARGE_THRESHOLD}.`)
  ...
  return {
    ...
    series: series.map(item => ({
      ...
      data: item.points,
      symbol: 'circle',
      // The optional third element of a point is its magnitude, mapped to area.
...
export function createScatter(el, data, tokens = readDatavTokens(el)) {
  let chart = null
  let overlay = null
  ...
  function degrade() {
    ...
    chart = null
    overlay = null
    const value = (data.series?.[0]?.points || [])[0]?.[1] ?? 0
