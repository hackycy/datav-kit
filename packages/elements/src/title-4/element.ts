...
// this in step with the `--dvk-title-4-title-gap` default — it is the value a measured
...
export class Title4Element extends DatavElement {
  static override styles = css`
    :host {
      ...
      min-width: 0;
      min-height: 0;
      color: var(--dvk-title-4-title-color, #f4fbff);
    }
    ...
    .content {
      ...
      top: var(--dvk-title-4-title-top, 37.33%);
    ...
    .title {
      ...
      padding: 0 var(--dvk-title-4-title-gap, 1.11em);
      overflow: hidden;
      color: var(--dvk-title-4-title-color, #f4fbff);
      font: var(--dvk-title-4-title-font, 700 var(--dvk-title-4-title-size, 42px)/1 'Microsoft YaHei', 'PingFang SC', 'Noto Sans CJK SC', Arial, sans-serif);
      letter-spacing: var(--dvk-title-4-title-letter-spacing, 0.2222em);
      text-align: center;
      white-space: nowrap;
      ...
      text-shadow:
        0 0.014em 0 var(--dvk-title-4-title-stroke, rgba(227, 245, 255, 0.28)),
        0 0.042em 0.069em var(--dvk-title-4-title-glow, rgba(0, 7, 15, 0.42));
    }
  ...
  private readonly instanceId = ++title4Id
  private readonly upperRailLeftId = `dvk-title-4-upper-rail-left-${this.instanceId}`
  private readonly upperRailRightId = `dvk-title-4-upper-rail-right-${this.instanceId}`
  private readonly ribbonLeftId = `dvk-title-4-ribbon-left-${this.instanceId}`
  private readonly ribbonRightId = `dvk-title-4-ribbon-right-${this.instanceId}`
  private readonly supportLeftId = `dvk-title-4-support-left-${this.instanceId}`
  private readonly supportRightId = `dvk-title-4-support-right-${this.instanceId}`
  private readonly horizonId = `dvk-title-4-horizon-${this.instanceId}`
  private readonly horizonHaloId = `dvk-title-4-horizon-halo-${this.instanceId}`
  private readonly hotCoreId = `dvk-title-4-hot-core-${this.instanceId}`
  private readonly railGlowId = `dvk-title-4-rail-glow-${this.instanceId}`
  private readonly haloBlurId = `dvk-title-4-halo-blur-${this.instanceId}`
  private readonly coreBlurId = `dvk-title-4-core-blur-${this.instanceId}`
  ...
  override render(): unknown {
    ...
    const glowOpacity = this.resolveOpacity('--dvk-title-4-glow-opacity', 1)
  ...
  private resolveTitleSizeVar(): string {
    const explicit = resolveThemeValue<string>({
      cssVariable: '--dvk-title-4-title-size',
      host: this,
      fallback: '',
    })

    if (explicit)
      return `--dvk-title-4-title-size: ${explicit}`

    return this.titleSize > 0 ? `--dvk-title-4-title-size: ${Math.round(this.titleSize * 100) / 100}px` : ''
  }
