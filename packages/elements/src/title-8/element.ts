...
//
// The matching clearance is `--dvk-title-8-title-gap`: the design's title box is 8 CJK
...
// exactly — the same kind of trade title-6 documents for its doubled gap.
...
export class Title8Element extends DatavElement {
  static override styles = css`
    :host {
      ...
      min-width: 0;
      min-height: 0;
      color: var(--dvk-title-8-title-color, #f4fbff);
    }
    ...
    .content {
      ...
      top: var(--dvk-title-8-title-top, 60.06%);
    ...
    .title {
      ...
      width: var(--dvk-title-8-title-width, max-content);
      max-width: 100%;
      /* The prototype's own clearance, so the frame lands on its design coordinates. */
      padding: 0 var(--dvk-title-8-title-gap, 1.435em);
      overflow: hidden;
      /* The prototype gives the glyphs a vertical white-to-blue ramp rather than a flat
         fill, which title-5 and title-6 both dropped. It survives here because it is pure
      ...
      color: var(--dvk-title-8-title-color, transparent);
      background-image: var(--dvk-title-8-title-gradient, linear-gradient(180deg, #fcfcfd 0%, #f8fbfd 25%, #edf7fd 43%, #cfe9fa 62%, #9ed1f2 79%, #63afe3 100%));
      -webkit-background-clip: text;
      background-clip: text;
      font: var(--dvk-title-8-title-font, 900 var(--dvk-title-8-title-size, 44px)/1 'Microsoft YaHei', 'PingFang SC', 'Noto Sans CJK SC', Arial, sans-serif);
      letter-spacing: var(--dvk-title-8-title-letter-spacing, 0.105em);
      text-align: center;
      white-space: nowrap;
      ...
      -webkit-text-stroke: var(--dvk-title-8-title-stroke-width, .0094em) var(--dvk-title-8-title-stroke, rgba(238, 249, 255, 0.64));
      ...
      filter: drop-shadow(0 .0132em 0 var(--dvk-title-8-title-glow, rgba(255, 255, 255, 0.2))) drop-shadow(0 .0263em .0263em var(--dvk-title-8-title-halo, rgba(61, 190, 249, 0.18))) drop-shadow(0 .0658em .0921em var(--dvk-title-8-title-shadow, rgba(36, 155, 229, 0.13)));
    }
  ...
  private readonly instanceId = ++title8Id
  private readonly bloomId = `dvk-title-8-bloom-${this.instanceId}`
  private readonly bandLeftId = `dvk-title-8-band-left-${this.instanceId}`
  private readonly bandRightId = `dvk-title-8-band-right-${this.instanceId}`
  private readonly railLeftId = `dvk-title-8-upper-rail-left-${this.instanceId}`
  private readonly railRightId = `dvk-title-8-upper-rail-right-${this.instanceId}`
  private readonly lowerLeftId = `dvk-title-8-lower-rail-left-${this.instanceId}`
  private readonly lowerRightId = `dvk-title-8-lower-rail-right-${this.instanceId}`
  private readonly slashLeftId = `dvk-title-8-slash-left-${this.instanceId}`
  private readonly slashRightId = `dvk-title-8-slash-right-${this.instanceId}`
  private readonly auraId = `dvk-title-8-aura-${this.instanceId}`
  private readonly spineId = `dvk-title-8-spine-${this.instanceId}`
  private readonly railGlowId = `dvk-title-8-rail-glow-${this.instanceId}`
  private readonly lineGlowId = `dvk-title-8-line-glow-${this.instanceId}`
  ...
  override firstUpdated(): void {
    this.emit('dvk-ready', { tagName: 'dvk-title-8' })
    this.observeTitle()
  }
  ...
  override render(): unknown {
    ...
    const glowOpacity = this.resolveOpacity('--dvk-title-8-glow-opacity', 1)
  ...
  private resolveTitleSizeVar(): string {
    const explicit = resolveThemeValue<string>({
      cssVariable: '--dvk-title-8-title-size',
      host: this,
      fallback: '',
    })

    if (explicit)
      return `--dvk-title-8-title-size: ${explicit}`

    return this.titleSize > 0 ? `--dvk-title-8-title-size: ${Math.round(this.titleSize * 100) / 100}px` : ''
  }
  ...
  private resolveTitleTopVar(): string {
    const explicit = resolveThemeValue<string>({
      cssVariable: '--dvk-title-8-title-top',
      host: this,
      fallback: '',
    })

    if (explicit)
      return `--dvk-title-8-title-top: ${explicit}`
    ...
    return `--dvk-title-8-title-top: ${Math.round(top * 100) / 100}px`
  }

  private resolveRecess(): string {
    return resolveThemeValue<string>({
      cssVariable: '--dvk-title-8-recess',
      host: this,
