import type { DatavElementMetadata } from '@datav-kit/core'
import { borderBox1Metadata } from './border-box-1/metadata'
import { borderBox2Metadata } from './border-box-2/metadata'
import { borderBox3Metadata } from './border-box-3/metadata'
import { borderBox4Metadata } from './border-box-4/metadata'
import { borderBox5Metadata } from './border-box-5/metadata'
import { borderBox6Metadata } from './border-box-6/metadata'
import { borderBox7Metadata } from './border-box-7/metadata'
import { borderBox8Metadata } from './border-box-8/metadata'
import { borderBox9Metadata } from './border-box-9/metadata'
import { borderBox10Metadata } from './border-box-10/metadata'
import { borderBox11Metadata } from './border-box-11/metadata'
import { borderBox12Metadata } from './border-box-12/metadata'
import { borderBox13Metadata } from './border-box-13/metadata'
import { borderBox14Metadata } from './border-box-14/metadata'
import { borderBox15Metadata } from './border-box-15/metadata'
import { borderBox16Metadata } from './border-box-16/metadata'
import { countToMetadata } from './count-to/metadata'
import { decoration1Metadata } from './decoration-1/metadata'
import { decoration2Metadata } from './decoration-2/metadata'
import { decoration3Metadata } from './decoration-3/metadata'
import { decoration4Metadata } from './decoration-4/metadata'
import { decoration5Metadata } from './decoration-5/metadata'
import { decoration6Metadata } from './decoration-6/metadata'
import { decoration7Metadata } from './decoration-7/metadata'
import { decoration8Metadata } from './decoration-8/metadata'
import { decoration9Metadata } from './decoration-9/metadata'
import { decoration10Metadata } from './decoration-10/metadata'
import { decoration11Metadata } from './decoration-11/metadata'
import { fitScreenMetadata } from './fit-screen/metadata'
import { loadingEnergyMetadata } from './loading-energy/metadata'
import { loadingOrbitMetadata } from './loading-orbit/metadata'
import { performanceMonitorMetadata } from './performance-monitor/metadata'
import { title1Metadata } from './title-1/metadata'
import { title2Metadata } from './title-2/metadata'

export type ElementMetadata = DatavElementMetadata

export const elementMetadata: ElementMetadata[] = [
  fitScreenMetadata,
  borderBox1Metadata,
  borderBox2Metadata,
  borderBox3Metadata,
  borderBox4Metadata,
  borderBox5Metadata,
  borderBox6Metadata,
  borderBox7Metadata,
  borderBox8Metadata,
  borderBox9Metadata,
  borderBox10Metadata,
  borderBox11Metadata,
  borderBox12Metadata,
  borderBox13Metadata,
  borderBox14Metadata,
  borderBox15Metadata,
  borderBox16Metadata,
  decoration1Metadata,
  decoration2Metadata,
  decoration3Metadata,
  decoration4Metadata,
  decoration5Metadata,
  decoration6Metadata,
  decoration7Metadata,
  decoration8Metadata,
  decoration9Metadata,
  decoration10Metadata,
  decoration11Metadata,
  title1Metadata,
  title2Metadata,
  countToMetadata,
  loadingOrbitMetadata,
  loadingEnergyMetadata,
  performanceMonitorMetadata,
]
