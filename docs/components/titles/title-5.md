...
<div class="datav-demo datav-demo--decoration" style="--datav-decoration-width: 640px; --datav-decoration-height: 64px;">
  <div class="datav-decoration-shell">
    <dvk-title-5 title-text="企业运营监控中心"></dvk-title-5>
  </div>
</div>
...
| `--dvk-title-5-accent` | Accent fallback color for the white-hot centre of the spine. |
| `--dvk-title-5-glow-opacity` | Multiplier on the blurred rail glow and the spine's glow rect. Defaults to `1`, which keeps the design's own glow weights. |
| `--dvk-title-5-recess` | Fill of the dark notch under each rail. Defaults to `rgba(4, 16, 30, 0.72)`, which is *darker* than the page — the notch is a cut, not a panel. |
| `--dvk-title-5-title-top` | Vertical position of the title box. When unset the component computes it from the host's aspect so the box's bottom sits at `146.1` of the 180-unit canvas, `24` units above the spine. Anchoring the bottom is what stops a band stretched taller than the design from drifting the title up and opening the gap beneath it, since the font tracks the host width alone. An explicit length or percentage always wins. In an environment that cannot measure the host, `60.06%` is used. |
| `--dvk-title-5-title-width` | Title box width, never exceeding the host. When unset the box follows its own text, and its measured width sets the frame. |
| `--dvk-title-5-title-gap` | Padding between the title text and the title box edge, so the shoulder sits one gap beyond the text. Any CSS length, defaulting to `1.435em` so it scales with the font. This is the prototype's own clearance, kept verbatim so the frame lands on its design coordinates. |
| `--dvk-title-5-title-size` | Title font size. When unset it follows the host: `76 × min(hostHeight / 180, hostWidth / 2048)`. An explicit value always wins. |
| `--dvk-title-5-title-font` | Title font shorthand. Replaces the family, weight, size, and line-height in one declaration. |
| `--dvk-title-5-title-letter-spacing` | Title letter spacing. Defaults to the prototype's wide `0.105em`. |
| `--dvk-title-5-title-gradient` | Background image clipped to the glyphs — the prototype's white-to-blue vertical ramp. Set it to `none` to fall back to a flat fill, which is what `--dvk-title-5-title-color` then paints. |
| `--dvk-title-5-title-color` | Flat title text color, used when the gradient is `none`. |
| `--dvk-title-5-title-stroke-width` | Width of the glyph outline stroke. Defaults to `.0094em`, the prototype's viewport-relative value at the design font. |
| `--dvk-title-5-title-stroke` | Color of that glyph outline stroke. |
| `--dvk-title-5-title-glow` | Tight white lift directly under the glyphs. |
| `--dvk-title-5-title-halo` | Mid cyan halo behind the glyphs. |
| `--dvk-title-5-title-shadow` | Wide blue depth shadow behind the glyphs. |
