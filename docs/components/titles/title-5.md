...
| `--dvk-title-5-title-width` | Title box width, never exceeding the host. When unset the box follows its own text, and its measured width sets the recess. |
| `--dvk-title-5-title-gap` | Padding between the title text and the recess edge. Any CSS length, defaulting to `2em` so it scales with the title font. The measured box includes it, so the recess sits one gap beyond the text. |
